   ...
   mapping(uint =>mapping(address => bool))public hascontributed; 
    ...
    function joinPool(uint poolId) external {
        ...
        //require(token.balanceOf(msg.sender) > pool.contributionPerParticipant * 2, "Contribution amount must be twice the amount per participant");
         require(mgmt.isBlacklisted(msg.sender) ==false, "You are blacklisted from the pool for defaulting");
    ...
    function isParticipantOfPool(uint poolId,address user) internal view returns(bool) {
    require(poolId < poolCounter, "Invalid pool ID");
    Pool storage pool = pools[poolId];
...
function _contributeToPool(uint poolId,uint _amount) internal {
    ...
    require(_amount == pool.contributionPerParticipant, "Contribution amount must match the specified amount per participant");
...
function claimTurn(uint poolId) external {
    ...
    Pool storage pool = pools[poolId];
    require(pool.active, "Pool is not active");
    require(isParticipantOfPool(poolId, msg.sender), "You are not a participant of this pool");
...
require(turn[poolId].endTime <= block.timestamp,"waitfor turn to end");
 ...
 _updateTurn(poolId, _time,msg.sender);
    ...
    emit TurnClaimed(poolId,pool.currentTurn, msg.sender, amountToTransfer);
}
//update turn
...
//getTurns
    function getTurnDetails(uint poolId) external view returns (TurnDetails memory) {
    require(poolId < poolCounter, "Invalid pool ID");
    return turn[poolId];
}
}